...
function Wo_ShareFile($data = array(), $type = 0, $crop = true){
    ...
    global $wo, $sqlConnect, $s3; $allowed = '';
    
    if($data['size'] > $wo['config']['maxUpload']){return false;}
                       
    $data['is_video'] = (empty($data['is_video'])) ? 0 : $data['is_video'];
    $data['file'] = ($data['file']) ?? null;
    $data['name'] = (!empty($data['name'])) ? Wo_Secure($data['name']) : null;
    $new_string     = pathinfo($data['name'], PATHINFO_FILENAME) . '.' . strtolower(pathinfo($data['name'], PATHINFO_EXTENSION));
    $file_extension = pathinfo($new_string, PATHINFO_EXTENSION); $file_extension = strtolower($file_extension);
    
    if(!file_exists($fY  = 'upload/files/' . date('Y')))                        {@mkdir($fY, 0777, true);}
    if(!file_exists($fYm = 'upload/files/' . date('Y') . '/' . date('m')))      {@mkdir($fYm, 0777, true);}
    
    if(!file_exists($pY  = 'upload/photos/' . date('Y')))                       {@mkdir($pY, 0777, true);}
    if(!file_exists($pYm = 'upload/photos/' . date('Y') . '/' . date('m')))     {@mkdir($pYm, 0777, true);}
    
    if(!file_exists($vY  = 'upload/videos/' . date('Y')))                       {@mkdir($vY, 0777, true);}
    if(!file_exists($vYm = 'upload/videos/' . date('Y') . '/' . date('m')))     {@mkdir($vYm, 0777, true);}
    
    if(!file_exists($sY  = 'upload/sounds/' . date('Y')))                       {@mkdir($sY, 0777, true);}
    if(!file_exists($sYm = 'upload/sounds/' . date('Y') . '/' . date('m')))     {@mkdir($sYm, 0777, true);}

    if(0 == $data['is_video']){
        $allowed = (1 == $wo['config']['fileSharing']) ? ($data['types'] ?? $wo['config']['allowedExtenstion']) : 'jpg,png,jpeg,gif,mp4,m4v,webm,flv,mov,mpeg,mp3,wav,mkv';
        $extension_allowed = explode(',', $allowed);
    ...
    if(in_array($file_extension, ['jpg', 'jpeg', 'png', 'gif'])){
        ...
        $fileType = 'image';
        $dir = $pYm;} else 
    if(in_array($file_extension, ['mp4', 'mov', 'webm', 'flv', 'mkv']) || 1 == $data['is_video']){
        $folder   = 'videos';
        $fileType = 'video';
        $dir = $vYm;} else
    if(in_array($file_extension, ['mp3', 'wav'])){
        ...
        $fileType = 'file';
        $dir = $fYm;
    }if(empty($folder) || empty($fileType) || empty($dir)){return false;}
        
    if(0 == $data['is_video']){
        $mime_types = explode(',', str_replace(' ', '', $wo['config']['mime_types'] . ',application/json,application/octet-stream'));
        if(Wo_IsAdmin()){$mime_types = explode(',', str_replace(' ', '', $wo['config']['mime_types'] . ',application/json,application/octet-stream,image/svg+xml'));}
...
function Wo_RegisterPost($re_data = array('recipient_id' => 0)){
    ...
    //
    /*/pa($sqlConnect); 
    pa(preg_quote($wo['config']['deepsound_url'])); 
    
    pa(get_loaded_extensions()); 
    
    
    pa(mysqli_query($sqlConnect, "SHOW DATABASES;"));
    pa(mysqli_fetch_array(mysqli_query($sqlConnect, "SHOW DATABASES;")));
    pa("INSERT INTO " . T_POSTS . " ({$fields}) VALUES ({$data})"); 
    pa(T_CONFIG); ///*/
