...
///*/ ahilespelid ///*/
        ...
        $image_array = array(); $wo['add_watermark'] = true;
        
        if(!Wo_CheckSession($hash_id)){return false; exit;}
        ...
        if($wo['config']['who_upload'] == 'pro' && $wo['user']['is_pro'] == 0 && !Wo_IsAdmin() && (!empty($_FILES['postFile']) || !empty($_FILES['postVideo']) || !empty($_FILES['postMusic']) || !empty($_FILES['postPhotos']) || !empty($_POST['postRecord']))){
            header("Content-type: application/json");
            echo json_encode(['status' => 400,'errors' => $errors = $wo['lang']['please_upgrade_to_upload']]);
            exit;
        }
        
        if(!Wo_CheckIfUserCanPost($wo['config']['post_limit'])){
            header("Content-type: application/json");
            echo json_encode(['status' => 400,'errors' => $errors = $wo['lang']['limit_exceeded']]);
            exit;
        }
