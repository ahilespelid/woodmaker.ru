...
if ($f == 'posts') {
    ...
    if ($s == 'insert_new_post') {
        ...
        if (isset($_FILES['postVideo']['name']) && empty($mediaFilename)) {
            ...
            if ($_FILES['postVideo']['size'] > $wo['config']['maxUpload']) {
               ...
               $fileInfo = array(
                ...
                if ($wo['config']['ffmpeg_system'] == 'off' && $wo['config']['cloudflare_stream'] == 'on') {

                    $videofile = Wo_ShareFile($fileInfo);

                    $videoFile = $videofile['filename'];
                    // $bearer_token = '2rXNrUjH1Yx5AZviT58X7PFc_RDEmh4h-OM7rcjk';
                    // $account_id = '51b4b48946573357d4b0b0f1fb6f2a6f';
                    $bearer_token = $wo['config']['bearer_cloudflare'];
                    $account_id = $wo['config']['account_id_cloudflare'];
                // $videoFile = 'upload/videos/2023/01/77MvJhJbLafr6QROpGJO_27_3355259be2f15dd584fa4c6c4c361d56_video.mp4';

                    $curl = curl_init();

                    curl_setopt_array($curl, [
                        CURLOPT_URL => 'https://api.cloudflare.com/client/v4/accounts/'.$account_id.'/stream',
                    ...
                    if ($err) {
                        ...
                        $mediaFilename = $resp['result']['playback']['hls'];
                        $video_thumb = $resp['result']['uid'];
                    }

                } else {
            //     exit();
                    if ($wo['config']['ffmpeg_system'] == 'on' && $wo['config']['cloudflare_stream'] == 'off') {
                        if ($not_video == false) {
                            $fileInfo['is_video'] = 1;
                        }
        ...
        $post_map  = '';
        $search = 'facebook';
        if(preg_match("/{$search}/i", $_POST['postText'])) {
            exit();
        }
        $search = 'instagram';
        if(preg_match("/{$search}/i", $_POST['postText'])) {
            ...
            if (!empty($wo['story']['blog_id']) && $wo['story']['blog_id'] > 0) {
                if (Wo_DeleteMyBlog($wo['story']['blog_id'])) {
                 ...
                 if ($wo['config']['ffmpeg_system'] == 'off' && $wo['config']['cloudflare_stream'] == 'on') {  
                    ...
                    $videoId = $wo['story']['postFileThumb'];
                        $bearerToken = $wo['config']['bearer_cloudflare'];
                        $accountId = $wo['config']['account_id_cloudflare'];


                        // Build the URL for the API request
                        $url = "https://api.cloudflare.com/client/v4/accounts/" . $accountId . "/stream/" . $videoId;
