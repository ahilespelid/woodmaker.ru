...
function Wo_DisplaySharedFile($media, $placement = '', $cache = false, $is_video = false) {
    ...
    if (!$is_video && $wo['config']['cloudflare_stream']) {
        $wo['media']['filename'] = $media['filename'];
    } else if (!$is_video) {
        $wo['media']['filename'] = Wo_GetMedia($media['filename']);
    }

    if ($wo['config']['cloudflare_stream']) { 
        $wo['media']['video_thumb'] = ((!empty($media['postFileThumb'])) ? $media['postFileThumb'] : '');
    } else {
        ...
        if ($file_extension == 'mp4' || $file_extension == 'mkv' || $file_extension == 'avi' || $file_extension == 'webm' || $file_extension == 'mov' || $file_extension == 'm3u8' || $is_video) {
            if ($placement == 'message' || $placement == 'chat') {
                ...
                if ($file_extension == 'm3u8') {
                 //  var_dump($wo['media']['filename']);
                    $wo['media']['filename'] = $wo['media']['filename'];
...
function Wo_CountUserPosts($user_id) {
    ...
    $user_id = Wo_Secure($user_id);
    $query   = mysqli_query($sqlConnect, "SELECT COUNT(`id`) AS count FROM " . T_POSTS . " WHERE `user_id` = {$user_id} AND `multi_image_post` != 1");
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        return $fetched_data['count'];
    }
